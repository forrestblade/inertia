...
export function getEventCountsByCategory (
  pool: DbPool, category: string | string[], start: Date, end: Date
): ResultAsync<ReadonlyArray<CategoryCount>, DbError> {
  const categories = Array.isArray(category) ? category : [category]
  return ResultAsync.fromPromise(
    pool.sql<CategoryCount[]>`
      SELECT dom_target, COUNT(*)::int AS count FROM events
      WHERE event_category = ANY(${pool.sql.array(categories)})
        AND created_at BETWEEN ${start} AND ${end}
      GROUP BY dom_target ORDER BY count DESC
    `, mapPostgresError
  ).map((rows) => rows as ReadonlyArray<CategoryCount>)
}

export function getPageviewsByPath (
  pool: DbPool, start: Date, end: Date, limit = 20
): ResultAsync<ReadonlyArray<PageviewCount>, DbError> {
  return ResultAsync.fromPromise(
    pool.sql<PageviewCount[]>`
      SELECT payload->>'path' AS path, COUNT(*)::int AS views FROM events
      WHERE event_category = 'PAGEVIEW'
        AND created_at BETWEEN ${start} AND ${end}
      GROUP BY payload->>'path' ORDER BY views DESC LIMIT ${limit}
    `, mapPostgresError
  ).map((rows) => rows as ReadonlyArray<PageviewCount>)
}

export function getDailyEventCounts (
  pool: DbPool, start: Date, end: Date, categories?: string[]
): ResultAsync<ReadonlyArray<DailyEventCount>, DbError> {
  if (categories !== undefined && categories.length > 0) {
    return ResultAsync.fromPromise(
      pool.sql<DailyEventCount[]>`
        SELECT DATE(created_at)::text AS day, event_category, dom_target, COUNT(*)::int AS count
        FROM events WHERE created_at BETWEEN ${start} AND ${end}
          AND event_category = ANY(${pool.sql.array(categories)})
        GROUP BY DATE(created_at), event_category, dom_target ORDER BY day DESC, count DESC
      `, mapPostgresError
