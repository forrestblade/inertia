...
Telemetry engine, HTML-over-the-wire router, and server-side routing primitives. The three critical subsystems in Inertia.
...
- Fragment protocol: `X-Inertia-Fragment: 1` header for partial responses.
...
- `isFragmentRequest()` checks `X-Inertia-Fragment: 1` header for fragment vs full-page dispatch.
